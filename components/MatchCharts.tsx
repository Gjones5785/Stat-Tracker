...
export const MatchCharts: React.FC<MatchChartsProps> = ({ matchData }) => {
  ...
  const handleSort = (key: string) => {
    setSortConfig(prev => {
      ...
      // Defaults: Numbers/Names ASC, Stats DESC
    ...
    return data.sort((a, b) => {
      ...
      switch(key) {
        ...
        case 'name':
          ...
          break;
        case 'impact':
          valA = calculateImpactScore(a);
          valB = calculateImpactScore(b);
          break;
        default:
          // It's a stat key
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Tab Switcher */}
      <div className="shrink-0 flex justify-center py-6 px-6">
             ...
             <div className="bg-white dark:bg-[#1A1A1C] rounded-3xl p-1 shadow-apple dark:shadow-none border border-gray-100 dark:border-white/5 inline-block min-w-full align-middle">
                  <table className="w-full">
                      <thead className="bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-white/5">
                         <tr>
                            <th 
                              className="p-3 text-left w-16 bg-gray-50 dark:bg-[#1A1A1C] text-xs font-bold text-gray-500 uppercase tracking-wider pl-4 cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors sticky top-0 z-20"
                              onClick={() => handleSort('number')}
                            >
                              <div className="flex items-center space-x-1">
                                <span>#</span>
                                {sortConfig.key === 'number' && <span className="text-red-500">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                              </div>
                            </th>
                            <th 
                              className="p-3 text-left min-w-[160px] bg-gray-50 dark:bg-[#1A1A1C] text-xs font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors sticky top-0 z-20"
                              onClick={() => handleSort('name')}
                            >
                              <div className="flex items-center space-x-1">
                                <span>Player</span>
                                {sortConfig.key === 'name' && <span className="text-red-500">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                              </div>
                            </th>
                            {Object.values(STAT_CONFIGS).map(cfg => (
                               <th 
                                 key={cfg.key} 
                                 className="p-3 text-center min-w-[90px] bg-gray-50 dark:bg-[#1A1A1C] text-xs font-bold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors sticky top-0 z-20"
                                 onClick={() => handleSort(cfg.key)}
                               >
                                  <div className="flex items-center justify-center space-x-1">
                                    <span>{cfg.label}</span>
                                    {sortConfig.key === cfg.key && <span className="text-red-500">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                                  </div>
                               </th>
                            ))}
                            <th 
                              className="p-3 text-center min-w-[100px] bg-gray-50 dark:bg-[#1A1A1C] text-xs font-bold text-yellow-600 dark:text-yellow-500 uppercase tracking-wider border-l border-gray-200 dark:border-white/5 cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors sticky top-0 z-20"
                              onClick={() => handleSort('impact')}
                            >
                              <div className="flex items-center justify-center space-x-1">
                                <span>Impact</span>
                                {sortConfig.key === 'impact' && <span className="text-red-500">{sortConfig.direction === 'asc' ? '↑' : '↓'}</span>}
                              </div>
                            </th>
                         </tr>
                      </thead>
