                  ...
                  {history.map((match) => (
                    <div key={match.id} className="group bg-white dark:bg-[#1A1A1C] rounded-2xl p-5 shadow-apple dark:shadow-none hover:shadow-apple-hover transition-all flex items-center justify-between border border-transparent hover:border-gray-100 dark:hover:border-white/5">
                      <div className="flex-1 min-w-0 cursor-pointer" onClick={() => onViewMatch(match)}>
                         <div className="flex items-center space-x-3 mb-1.5"><span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{match.date}</span><span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${match.result === 'win' ? 'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400' : match.result === 'loss' ? 'bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400' : match.result === 'draw' ? 'bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400' : 'bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-400'}`}>{match.result}</span></div>
                         <h4 className="text-xl font-heading font-bold text-slate-900 dark:text-white truncate">{match.opponentName || 'Unknown Opponent'}</h4>
                         <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mt-0.5">{match.teamName} <span className="mx-2 text-gray-300 dark:text-gray-600">|</span> <span className="font-mono text-slate-700 dark:text-gray-300">{match.finalScore}</span></p>
                      </div>
                      <div className="flex items-center space-x-2 pl-4 border-l border-gray-100 dark:border-white/5"><Button variant="secondary" onClick={() => onViewMatch(match)} className="text-xs px-4 py-2 rounded-xl bg-gray-50 dark:bg-white/5 hover:bg-gray-100 dark:hover:bg-white/10 text-slate-700 dark:text-gray-300 border-none font-semibold">View</Button><button type="button" onClick={(e) => handleDeleteClick(e, match.id)} className="relative z-20 p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors focus:outline-none focus:ring-2 focus:ring-red-500" title="Delete Match"><svg className="w-5 h-5 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button></div>
                    </div>
                  ))}
                </div>
              )}
