      ...
      <header className="bg-white/80 dark:bg-[#1A1A1C]/80 backdrop-blur-md sticky top-0 z-[100] border-b border-gray-200/50 dark:border-white/5 supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-[#1A1A1C]/60">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-3 shrink-0 pr-4">
             {logo ? (
               <img src={logo} alt="Logo" className="w-9 h-9 rounded-xl object-contain shadow-sm bg-white border border-gray-100 dark:border-transparent dark:bg-white" />
             ) : (
              ...
              {history.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                  {history.map((match) => {
                    ...
                    return (
                      <div key={match.id} className={`group bg-white dark:bg-[#1A1A1C] rounded-2xl shadow-apple dark:shadow-none hover:shadow-apple-hover transition-all flex items-center border border-transparent hover:border-gray-100 dark:hover:border-white/5 h-24 relative ${isMenuOpen ? 'z-[105]' : 'z-0'}`}>
                        ...
                        <div className="flex-1 px-6 flex items-center justify-between min-w-0 h-full">
                          ...
                          <div className="shrink-0 pl-4 border-l border-gray-100 dark:border-white/5 flex items-center space-x-1 sm:space-x-2 h-10">
                            ...
                            <div className="relative">
                              <button type="button" onClick={(e) => { e.stopPropagation(); setOpenMenuMatchId(isMenuOpen ? null : match.id); }} className="w-9 h-9 flex items-center justify-center text-gray-400 hover:text-slate-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-all" title="Actions"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z" /></svg></button>
                              {isMenuOpen && (
                                <>
                                  <div className="fixed inset-0 z-[110]" onClick={(e) => { e.stopPropagation(); setOpenMenuMatchId(null); }} />
                                  <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#1A1A1C] border border-gray-100 dark:border-white/10 rounded-2xl shadow-2xl z-[120] overflow-hidden py-1">
                                     <button 
                                       onClick={(e) => { e.stopPropagation(); handleDeleteRequest(match.id); }}
                                       className="w-full flex items-center px-4 py-4 text-xs font-black uppercase text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                                     >
                                       <svg className="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                       Delete Match
                                     </button>
                                  </div>
                                </>
