...
const PlayerCardModal: React.FC<PlayerCardModalProps> = ({ player, history, onClose }) => {
  ...
  const getRating = (val: number, maxExpected: number) => Math.min(99, Math.max(10, Math.round((val / maxExpected) * 99)));

  const attributes = {
    RUN: getRating(playerStats.avg.hitUps, 15),          
    TRY: getRating(playerStats.avg.tries * 2 + playerStats.avg.lineBreaks, 3), 
    PAS: getRating(playerStats.avg.assists * 3 + playerStats.avg.offloads, 4), 
    DEF: getRating(playerStats.avg.tackles, 25),         
    PHY: getRating(playerStats.avg.impact, 40),          
    DIS: Math.max(10, 99 - Math.round((playerStats.avg.errors + playerStats.avg.penalties) * 15)) 
  };

  const overall = Math.round((attributes.RUN + attributes.TRY + attributes.PAS + attributes.DEF * 1.5 + attributes.PHY + attributes.DIS) / 6.5);

  let cardTheme = {
    bg: 'bg-gradient-to-br from-yellow-600 via-yellow-200 to-yellow-600', 
  ...
  if (overall < 65) {
    cardTheme = { bg: 'bg-gradient-to-br from-orange-700 via-orange-300 to-orange-700', border: 'border-orange-400', text: 'text-slate-900', glow: 'shadow-[0_0_30px_rgba(194,65,12,0.4)]' };
  } else if (overall < 75) {
    cardTheme = { bg: 'bg-gradient-to-br from-slate-400 via-slate-100 to-slate-400', border: 'border-slate-300', text: 'text-slate-900', glow: 'shadow-[0_0_30px_rgba(148,163,184,0.4)]' };
  } else if (overall >= 90) {
    cardTheme = { bg: 'bg-gradient-to-br from-blue-600 via-cyan-400 to-blue-800', border: 'border-cyan-400', text: 'text-white', glow: 'shadow-[0_0_40px_rgba(34,211,238,0.6)]' };
  }
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 flex flex-col items-center animate-in zoom-in-95 duration-300">
           ...
           <div className={`absolute inset-0 ${cardTheme.bg} p-[6px]`}>
              ...
              <div className="w-full h-full bg-slate-900/90 rounded-t-[20px] rounded-b-lg relative overflow-hidden flex flex-col">
                 <div className="flex p-5 relative z-10">
                    ...
                    <button onClick={() => setShowLegend(!showLegend)} className="absolute top-4 right-2 text-white/30 hover:text-white transition-colors p-2" title="What do these stats mean?">
                    ...
                    {showLegend ? (
                      ...
                      <div className="grid grid-cols-2 gap-2 px-5 pb-5 h-full">
                        <div className="flex flex-col justify-center space-y-1.5 text-sm">
                          <div className="flex items-center justify-between text-white/90"><span className="font-bold mr-2 text-white/60">RUN</span><span className="font-heading font-bold text-yellow-400">{attributes.RUN}</span></div>
                          <div className="flex items-center justify-between text-white/90"><span className="font-bold mr-2 text-white/60">DEF</span><span className="font-heading font-bold text-yellow-400">{attributes.DEF}</span></div>
                          <div className="flex items-center justify-between text-white/90"><span className="font-bold mr-2 text-white/60">PHY</span><span className="font-heading font-bold text-yellow-400">{attributes.PHY}</span></div>
                          <div className="flex items-center justify-between text-white/90"><span className="font-bold mr-2 text-white/60">TRY</span><span className="font-heading font-bold text-yellow-400">{attributes.TRY}</span></div>
                          <div className="flex items-center justify-between text-white/90"><span className="font-bold mr-2 text-white/60">PAS</span><span className="font-heading font-bold text-yellow-400">{attributes.PAS}</span></div>
                          <div className="flex items-center justify-between text-white/90"><span className="font-bold mr-2 text-white/60">DIS</span><span className="font-heading font-bold text-yellow-400">{attributes.DIS}</span></div>
                        </div>
                        <div className="flex items-center justify-center relative">
                            <svg viewBox="0 0 100 100" className="w-full h-full max-w-[120px] drop-shadow-[0_0_10px_rgba(0,0,0,0.5)]">
                              ...
                              <polygon points={polygonPoints} fill={overall >= 90 ? "rgba(34, 211, 238, 0.5)" : "rgba(250, 204, 21, 0.5)"} stroke={overall >= 90 ? "#22d3ee" : "#facc15"} strokeWidth="2" />
                              {chartData.map((d, i) => {
                                  const [x, y] = getPoint(115, i, 6).split(','); 
                                  return (<text key={i} x={x} y={y} fontSize="8" fill="rgba(255,255,255,0.5)" textAnchor="middle" dominantBaseline="middle" fontWeight="bold">{d.label[0]}</text>);
                              })}
                            </svg>
                        </div>
                      </div>
        ...
        <div className="mt-6 flex space-x-4">
           <button onClick={onClose} className="px-6 py-2 rounded-full bg-white/10 hover:bg-white/20 text-white font-bold backdrop-blur-sm transition-colors border border-white/10">Close</button>
           <button className="px-6 py-2 rounded-full bg-white text-slate-900 font-bold hover:scale-105 transition-transform shadow-lg flex items-center">
              <svg className="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg> Share Card
           </button>
        </div>
      </div>
    </div>
...
export const SquadStatsView: React.FC<SquadStatsViewProps> = ({
  ...
  // Confirmation state
  const [archivePlayerId, setArchivePlayerId] = useState<string | null>(null);
  const [deletePlayerId, setDeletePlayerId] = useState<string | null>(null);

  const squadSummary = useMemo(() => {
    const active = squad.filter(p => p.active !== false).length;
    const inactive = squad.filter(p => p.active === false).length;
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    onAddPlayer({ name: newName, position: newPosition, active: true });
    setNewName('');
    setNewPosition('');
  };
  ...
  const handleArchiveConfirm = () => {
    if (archivePlayerId) {
        onUpdatePlayer(archivePlayerId, { active: false });
        setArchivePlayerId(null);
    }
  };

  const handleDeleteConfirm = () => {
    if (deletePlayerId) {
        onRemovePlayer(deletePlayerId);
        setDeletePlayerId(null);
    }
  };

  const restorePlayer = (id: string) => {
    onUpdatePlayer(id, { active: true });
  };

  const displaySquad = useMemo(() => {
    let filtered = [...squad];
    if (showInactive) filtered = filtered.filter(p => p.active === false);
    else filtered = filtered.filter(p => p.active !== false);

    if (!rosterSort) return filtered;
  ...
  const careerStats = useMemo(() => {
    const statsMap: Record<string, { 
      ...
      name: string,
      wins: number,
      historyImpacts: number[],
      rawPlayer: SquadPlayer
    }> = {};

    const activeSquad = squad.filter(p => p.active !== false);
    ...
    history.forEach(match => {
      ...
      matchPlayers.forEach(mp => {
        ...
        if (!squadId) {
          const found = activeSquad.find(s => s.name.toLowerCase() === mp.name.toLowerCase());
        ...
        if (squadId && statsMap[squadId]) {
          ...
          Object.keys(finalStats).forEach(k => {
             ...
             if (typeof finalStats[key] === 'number') entry.stats[key] += finalStats[key];
          });
        }
      });
    });
    ...
    const aggregated = Object.entries(statsMap).map(([id, data]) => {
        ...
        const formRating = Math.min(99, Math.round((avgImpact / 35) * 99)); 
        const recentForm = data.historyImpacts.slice(-5);
        return { id, ...data, formRating: data.games > 0 ? formRating : 0, recentForm };
    });
    ...
    return aggregated.sort((a, b) => {
      ...
      else if (key === 'games') { valA = a.games; valB = b.games; } 
      else if (key === 'form') { valA = a.formRating; valB = b.formRating; } 
      else { valA = a.stats[key as keyof PlayerStats] || 0; valB = b.stats[key as keyof PlayerStats] || 0; }
  ...
  const getFormColor = (rating: number) => {
      if (rating >= 90) return 'bg-gradient-to-r from-blue-500 to-cyan-400 text-white shadow-[0_0_10px_rgba(34,211,238,0.5)]'; 
      if (rating >= 75) return 'bg-gradient-to-r from-yellow-500 to-amber-400 text-white'; 
      if (rating >= 60) return 'bg-gray-400 text-white'; 
      ...
      {activeTab === 'roster' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white dark:bg-[#1A1A1C] p-6 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm">
              ...
              <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                 <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Squad Summary</h4>
                 <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 dark:bg-white/5 p-3 rounded-xl border border-gray-100 dark:border-white/5">
                       <span className="block text-2xl font-heading font-black text-blue-600 dark:text-blue-400">{squadSummary.active}</span>
                       <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Active</span>
                    </div>
                    <div className="bg-gray-50 dark:bg-white/5 p-3 rounded-xl border border-gray-100 dark:border-white/5">
                       <span className="block text-2xl font-heading font-black text-gray-400 dark:text-gray-500">{squadSummary.inactive}</span>
                       <span className="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Archived</span>
                    </div>
                 </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white dark:bg-[#1A1A1C] rounded-xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden flex flex-col">
              <div className="px-6 py-4 border-b border-gray-200 dark:border-white/10 flex justify-between items-center bg-gray-50/50 dark:bg-white/5">
                 <span className="text-sm font-bold text-gray-500 uppercase tracking-widest">{showInactive ? 'Inactive / Archived Players' : 'Active Squad'}</span>
                 <button onClick={() => setShowInactive(!showInactive)} className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors flex items-center ${showInactive ? 'bg-gray-200 dark:bg-white/20 text-slate-800 dark:text-white border-transparent' : 'bg-white dark:bg-white/5 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-white/10 hover:bg-gray-50'}`}>
                   {showInactive ? 'Show Active Squad' : 'Show Inactive Players'}
                 </button>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
                  ...
                  <tbody className="bg-white dark:bg-[#1A1A1C] divide-y divide-gray-200 dark:divide-white/10">
                    {displaySquad.length === 0 ? (
                      <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400">{showInactive ? 'No inactive players found.' : 'No players in squad. Add one to get started.'}</td></tr>
                    ) : (
                                ...
                                <div className="flex justify-end space-x-2">
                                  ...
                                  {showInactive ? (
                                    <>
                                      ...
                                      <button onClick={() => setDeletePlayerId(player.id)} className="p-1.5 text-gray-400 hover:text-red-600 transition-colors rounded-full hover:bg-red-50 dark:hover:bg-red-900/20" title="Delete Permanently">
                                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                      </button>
                                    </>
                                  ) : (
                                    <button onClick={() => setArchivePlayerId(player.id)} className="p-1.5 text-gray-400 hover:text-orange-500 transition-colors rounded-full hover:bg-orange-50 dark:hover:bg-orange-900/20" title="Archive / Inactive">
                                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                    </button>
                                  )}
                                </div>
        ...
        <>
          <div className="bg-white dark:bg-[#1A1A1C] rounded-xl border border-gray-200 dark:border-white/10 shadow-sm overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 dark:divide-white/10">
              <thead className="bg-gray-50 dark:bg-white/5">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider sticky left-0 bg-gray-50 dark:bg-[#1A1A1C] cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors z-10" onClick={() => handleStatSort('name')}>
                    <div className="flex items-center space-x-1"><span>Player</span>{statSort.key === 'name' && <span className="text-red-500">{statSort.direction === 'desc' ? '↓' : '↑'}</span>}</div>
                  </th>
                  <th className="px-4 py-3 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors" onClick={() => handleStatSort('form')}>
                    <div className="flex items-center justify-center space-x-1"><span>Form</span>{statSort.key === 'form' && <span className="text-red-500">{statSort.direction === 'desc' ? '↓' : '↑'}</span>}</div>
                  </th>
                  <th className="px-4 py-3 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors" onClick={() => handleStatSort('games')}>
                    <div className="flex items-center justify-center space-x-1"><span>Matches</span>{statSort.key === 'games' && <span className="text-red-500">{statSort.direction === 'desc' ? '↓' : '↑'}</span>}</div>
                  </th>
                  {STAT_CONFIGS.map(sc => (
                    <th key={sc.key} onClick={() => handleStatSort(sc.key)} className="px-4 py-3 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider whitespace-nowrap cursor-pointer hover:bg-gray-100 dark:hover:bg-white/10 transition-colors">
                      <div className="flex items-center justify-center space-x-1"><span>{sc.label}</span>{statSort.key === sc.key && <span className="text-red-500">{statSort.direction === 'desc' ? '↓' : '↑'}</span>}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody className="bg-white dark:bg-[#1A1A1C] divide-y divide-gray-200 dark:divide-white/10">
                {careerStats.map(p => (
                  <tr key={p.id} onClick={() => setSelectedCardPlayer(p.rawPlayer)} className="hover:bg-blue-50 dark:hover:bg-white/10 cursor-pointer transition-colors" title="View Player Card">
                    <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-slate-900 dark:text-white sticky left-0 bg-white dark:bg-[#1A1A1C] border-r border-gray-100 dark:border-white/5 flex items-center">
                      <span className="w-1.5 h-1.5 rounded-full bg-blue-500 mr-2 opacity-0 group-hover:opacity-100 transition-opacity"></span>{p.name}
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-center">{p.games > 0 ? <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full w-8 inline-block text-center ${getFormColor(p.formRating)}`}>{p.formRating}</span> : <span className="text-xs text-gray-300">-</span>}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-center text-gray-600 dark:text-gray-400">{p.games}</td>
                    {STAT_CONFIGS.map(sc => (<td key={sc.key} className="px-4 py-3 whitespace-nowrap text-sm text-center text-slate-900 dark:text-gray-200 font-mono">{p.stats[sc.key]}</td>))}
                  </tr>
                ))}
                {careerStats.length === 0 && (<tr><td colSpan={STAT_CONFIGS.length + 3} className="px-6 py-12 text-center text-gray-400">No active player stats to show.</td></tr>)}
              </tbody>
            </table>
          </div>
          {selectedCardPlayer && (<PlayerCardModal player={selectedCardPlayer} history={history} onClose={() => setSelectedCardPlayer(null)} />)}
        </>
      )}

      {/* Confirmation Modals */}
      <ConfirmationModal 
        isOpen={!!archivePlayerId} 
        title="Archive Player?" 
        message="This player will be moved to the Inactive list. Their career stats will be hidden but not deleted." 
        onConfirm={handleArchiveConfirm} 
        onCancel={() => setArchivePlayerId(null)} 
      />
      <ConfirmationModal 
        isOpen={!!deletePlayerId} 
        title="Delete Permanently?" 
        message="Are you sure? This will permanently remove the player and all their history. This cannot be undone." 
