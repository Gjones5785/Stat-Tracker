...
export const MatchCharts: React.FC<MatchChartsProps> = ({ matchData }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <style>{`
        @media print {
          /* Force physical page dimensions and margins */
          @page { 
            size: portrait; 
            margin: 10mm 10mm 10mm 10mm !important; 
          }
          
          /* Force absolute parent layout resets to disable scrollbars & clipping across devices & paper sizes */
          html, body, #root, #root *, #root > div, .min-h-screen, .flex-1, main, .overflow-hidden, .overflow-y-auto, [class*="overflow-"], [class*="min-h-"], [class*="flex-"] {
          ...
          /* Keep .no-print and its descendants strictly hidden during print to avoid extra page generation */
          .no-print, .no-print *, header, nav, footer, .sticky, button, [role="tablist"], .tab-switcher, .screen-content {
          ...
          /* Force exact print-only visibility */
          .print-only {
            display: block !important;
          ...
          /* Prevent table rows/cards from being split awkwardly midway */
          .print-card {
            background: white !important;
          ...
          /* Match stats table formatting to fit beautifully on any width (A4/Letter/Legal) */
          table {
            width: 100% !important;
      ...
      <div className="print-only p-4 text-slate-900">
            ...
            <div className="print-card bg-yellow-50 rounded-2xl">
               ...
               {voting ? (
                 <div className="space-y-3">
                   <div className="flex justify-between items-center pb-2 border-b border-yellow-105"><span className="text-xs font-bold uppercase tracking-tight">3 Points</span><span className="text-sm font-black uppercase">{getVotedPlayerName(voting.threePointsId)}</span></div>
                   <div className="flex justify-between items-center pb-2 border-b border-yellow-105"><span className="text-xs font-bold uppercase tracking-tight">2 Points</span><span className="text-sm font-black uppercase">{getVotedPlayerName(voting.twoPointsId)}</span></div>
         ...
         <div className="page-break pt-6">
            ...
            <table className="w-full">
               <thead>
                  <tr className="bg-slate-100">
                     ...
                     <th className="text-[9px] uppercase text-center">Min</th>
                     <th className="text-[9px] uppercase text-center">TR</th>
                     <th className="text-[9px] uppercase text-center">TA</th>
                     <th className="text-[9px] uppercase text-center">LB</th>
                     <th className="text-[9px] uppercase text-center">OL</th>
                     <th className="text-[9px] uppercase text-center">HU</th>
                     <th className="text-[9px] uppercase text-center">K</th>
                     <th className="text-[9px] uppercase text-center">40</th>
                     <th className="text-[9px] uppercase text-center">FD</th>
                     <th className="text-[9px] uppercase text-center">T</th>
                     <th className="text-[9px] uppercase text-center">MT</th>
                     <th className="text-[9px] uppercase text-center">TS</th>
                     <th className="text-[9px] uppercase text-center">1S</th>
                     <th className="text-[9px] uppercase text-center">P</th>
                     <th className="text-[9px] uppercase text-center">E</th>
                     <th className="text-[9px] uppercase text-center">Imp</th>
                  </tr>
               </thead>
               <tbody>
                  {sortedPlayers.map(p => (
                     <tr key={p.id}>
                        ...
                        <td className="text-center font-mono">{Math.round((p.totalSecondsOnField || 0) / 60)}m</td>
                        <td className="text-center font-mono">{p.stats.triesScored || 0}</td>
                        <td className="text-center font-mono">{p.stats.tryAssists || 0}</td>
                        <td className="text-center font-mono">{p.stats.lineBreaks || 0}</td>
                        <td className="text-center font-mono">{p.stats.offloads || 0}</td>
                        <td className="text-center font-mono">{p.stats.hitUps || 0}</td>
                        <td className="text-center font-mono">{p.stats.kicks || 0}</td>
                        <td className="text-center font-mono">{p.stats.fortyTwenties || 0}</td>
                        <td className="text-center font-mono">{p.stats.forcedDropouts || 0}</td>
                        <td className="text-center font-mono">{p.stats.tackles || 0}</td>
                        <td className="text-center font-mono">{p.stats.missedTackles || 0}</td>
                        <td className="text-center font-mono">{p.stats.trySavers || 0}</td>
                        <td className="text-center font-mono">{p.stats.oneOnOneStrips || 0}</td>
                        <td className="text-center font-mono">{p.stats.penaltiesConceded || 0}</td>
                        <td className="text-center font-mono">{p.stats.errors || 0}</td>
            ...
            <div className="mt-6 grid grid-cols-4 gap-y-2 gap-x-6 border-t border-slate-200 pt-4 text-[8px] text-slate-500 font-bold uppercase tracking-wider">
               <div><strong>TR:</strong> Tries Scored</div>
               <div><strong>TA:</strong> Try Assists</div>
               <div><strong>LB:</strong> Line Breaks</div>
               <div><strong>OL:</strong> Offloads</div>
               <div><strong>HU:</strong> Hit-Ups</div>
               <div><strong>K:</strong> Kicks</div>
               <div><strong>40:</strong> 40/20 Kicks</div>
               <div><strong>FD:</strong> Forced Dropouts</div>
               <div><strong>T:</strong> Tackles Made</div>
               <div><strong>MT:</strong> Missed Tackles</div>
               <div><strong>TS:</strong> Try Savers</div>
               <div><strong>1S:</strong> 1-on-1 Strips</div>
               <div><strong>P:</strong> Penalties Conceded</div>
               <div><strong>E:</strong> Handling Errors</div>
               <div><strong>Min:</strong> Minutes Played</div>
               <div><strong>Imp:</strong> Performance Impact</div>
            </div>
         </div>
      </div>
